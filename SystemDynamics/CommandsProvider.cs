...
namespace SystemDynamics
{
    class CommandsProvider : IEnumerable<AbstractCommand>
    {
        ...
        public CommandsProvider(Drawer drawer = null)
        {
            ...
            {
                new commands.Edit<WaterIceSteamState, double>(drawer.state, "Отвечает за температуру плавления твёрдого вещества.", nameof(drawer.state.ТемператураПлавленияТвёрдогоВещества), (state, v) => state.ТемператураПлавленияТвёрдогоВещества = v, (state) => state.ТемператураПлавленияТвёрдогоВещества, double.TryParse, (d) => true),
                new commands.Edit<WaterIceSteamState, double>(drawer.state, "Отвечает за температуру кипения воды.", nameof(drawer.state.ТемператураКипенияЖидкости), (state, v) => state.ТемператураКипенияЖидкости = v, (state) => state.ТемператураКипенияЖидкости, double.TryParse, (d) => true),
                ...
                new commands.Edit<WaterIceSteamState, double>(drawer.state, "Отвечает за удельную теплоту парообразования и конденсации.", nameof(drawer.state.УдельнаяТеплотаПарообразования), (state, v) => state.УдельнаяТеплотаПарообразования = v, (state) => state.УдельнаяТеплотаПарообразования, double.TryParse, (d) => true),
                new commands.Edit<WaterIceSteamState, double>(drawer.state, "Отвечает за теплоёмкость твёрдого вещества.", nameof(drawer.state.ТеплоёмкостьТвёрдогоВещества), (state, v) => state.ТеплоёмкостьТвёрдогоВещества = v, (state) => state.ТеплоёмкостьТвёрдогоВещества, double.TryParse, (d) => true),
                ...
                new commands.Edit<WaterIceSteamState, double>(drawer.state, "Отвечает за теплоёмкость газа.", nameof(drawer.state.ТеплоёмкостьГаза), (state, v) => state.ТеплоёмкостьГаза = v, (state) => state.ТеплоёмкостьГаза, double.TryParse, (d) => true),
                ...
                new commands.Edit<WaterIceSteamState, double>(drawer.state, "Отвечает за текущее количество Джоуль в системе. Может быть только неотрицательное число.", nameof(drawer.state.КоличествоДжоулей), (state, v) => state.КоличествоДжоулей = v, (state) => state.КоличествоДжоулей, double.TryParse, (d) => d >= 0),
                new commands.Edit<WaterIceSteamState, double>(drawer.state, "Суммарная масса вещества. Не может быть равна нулю. Можно попробовать указать отрицательную массу.", nameof(drawer.state.СуммарнаяМасса), (state, v) => state.СуммарнаяМасса = v, (state) => state.СуммарнаяМасса, double.TryParse, (d) => d != 0),
                new commands.Edit<WaterIceSteamState, double>(drawer.state, "Отвечает за ускорение, замедление, поворот времени.", nameof(drawer.state.MultiplicationTime), (state, v) => state.MultiplicationTime = v, (state) => state.MultiplicationTime, double.TryParse, (d) => !double.IsNaN(d)),
                new commands.Edit<Drawer, int>(drawer, "Задаёт миллисекунды, с какой переодичностью обновлять систему. Диапазон значений: от 0 включительно до 1000 включительно. Допустимы только целые числа.", nameof(drawer.IntervalUpdate), (d, i) => d.IntervalUpdate = i, (d) => d.IntervalUpdate, int.TryParse, (d) => 0 <= d && d <= 1000),
